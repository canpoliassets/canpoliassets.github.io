...
pe_disclosures = mydb["pei_disclosures"]
pe_mlas = mydb["pei_mlas"]

# Set up the driver
driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()))
            
# Navigate to the webpage
url = "https://www.assembly.pe.ca/offices/conflict-of-interest-commissioner/mla-public-disclosure-statements"
driver.get(url)
...
for tag in a_tags:
    name = tag.text.replace("Hon. ", "")
    name = name.rstrip("\n")
    name = name.rstrip(" ")
    
    found = pe_mlas.find_one({'name': name })

    if not found:
        print("Error for", name, found)
        continue
...
for name in name_list:
    print(name, name_to_pdf_dict[name])
