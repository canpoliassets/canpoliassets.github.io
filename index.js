...
app.get('/:lang', async (req, res) => {
    ...
    const provinces = Object.entries(req.i18n.t("provinces")).sort((a, b) => req.collator(a[1], b[1]));
    const constituenciesByProvince = Object.groupBy(
        members
            .map(member => ({
                name: member.constituency,
                // TODO: yuck
                province: Object.entries(req.i18n.t("provinces", {}, "en")).find(province => province[1] === member.province)[0]
            }))
...
const PROVINCES = {
    ...
    qc: {
        ...
        mapDisclosures: member => {
            ...
            member.constituency_slug = slugify(member.constituency);
            return member;
        },
    },
};
